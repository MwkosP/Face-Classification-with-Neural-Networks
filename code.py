...
# 🔧 Ρυθμίσεις
#dataset_path   # ← άλλαξέ το με το σωστό path
phash_threshold = 5                # Πόσο κοντά πρέπει να είναι τα perceptual hashes
ssim_threshold = 0.90              # SSIM για να θεωρηθούν παρόμοιες

# 📌 Υπολογισμός phash για κάθε εικόνα στους υποφακέλους
...
# 📌 Ομαδοποίηση κοντινών hashes
potential_duplicates = []
paths = [item for sublist in hash_dict.values() for item in sublist]

print(f"🔍 Εικόνες για έλεγχο: {len(paths)}")
print("🧠 Έλεγχος για πιθανές διπλότυπες εικόνες...")
...
if potential_duplicates:
    print(f"\n🔗 Βρέθηκαν {len(potential_duplicates)} πιθανές διπλότυπες εικόνες:")
    for p1, p2, score in potential_duplicates:
        print(f"\n✅ SSIM: {score:.3f}")
        print(f"📁 {p1}")
        print(f"📁 {p2}")
else:
    print("✅ Δεν εντοπίστηκαν διπλότυπες εικόνες με τα κριτήρια που ορίστηκαν.")
...
plot_model(Model_Xception, show_shapes=True, show_layer_names=True, dpi=50, rankdir="LR")  # LR = Left to Right (οριζόντια)
...
plot_model(Model_InceptionV3, show_shapes=True, show_layer_names=True, dpi=50, rankdir="LR")  # LR = Left to Right (οριζόντια)
...
# Ξεκλείδωσε περισσότερα ή λιγότερα layers
for layer in xception_base.layers[-500:]:  # <-- άλλαξε τον αριθμό εδώ (π.χ. 5, 10, 20)
...
# Ξεκλείδωσε περισσότερα ή λιγότερα layers
for layer in base_model.layers[-300:]:  # <-- άλλαξε τον αριθμό εδώ (π.χ. 5, 10, 20)
    layer.trainable = True
